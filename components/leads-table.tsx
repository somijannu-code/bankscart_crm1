...
export function LeadsTable({ leads = [], telecallers = [] }: LeadsTableProps) {
  ...
  const [bulkAssignTo, setBulkAssignTo] = useState<string[]>([]) // New state for multi-assign
  ...
  // *** UPDATED: Bulk Assign function for Round-Robin distribution ***
  const handleBulkAssign = async () => {
    // Check if any telecallers or leads are selected
    // Uses the state `bulkAssignTo` which is an array of IDs
    if (bulkAssignTo.length === 0 || selectedLeads.length === 0) {
        setErrorMessage("Please select at least one lead and one telecaller for assignment.")
    ...
    try {
        ...
        const updates: Array<{ id: string, assigned_to: string, assigned_by: string | undefined, assigned_at: string }> = []
        ...
        selectedLeads.forEach((leadId, index) => {
            // Cycle through the selected telecaller IDs
        ...
        if (errors.length > 0) {
            // Use the component's error state to show a message
            setErrorMessage(`Failed to assign ${errors.length} leads. Please check the console for details.`)
            console.error("Bulk assign errors:", errors);
            // Throw to exit and ensure success message isn't shown
            throw new Error("Bulk assignment failed for some leads.") 
        }

        // Set success message and clear states
        setSuccessMessage(`Successfully bulk assigned ${selectedLeads.length} leads to ${telecallerIds.length} telecallers.`);
        setSelectedLeads([])
        setBulkAssignTo([])
        
        // Clear success/error messages after a delay for better UX
        setTimeout(() => {
          setSuccessMessage("");
          setErrorMessage("");
          window.location.reload(); // Hard refresh to show updates, as was in original function
        }, 500); 
        
    } catch (error) {
        // This catches both the thrown error and network errors
        console.error("Error bulk assigning leads:", error)
        if (!errorMessage.includes("Failed to assign")) { // Only set generic error if not already set by the error handling above
            setErrorMessage("An unexpected error occurred during bulk assignment.")
        }
        setTimeout(() => setErrorMessage(""), 5000); 
    }
  }
                    ...
                    <SelectContent>
                      <SelectItem value="round-robin">
                        <div className="flex flex-col">
                          ...
                          <span className="text-xs text-muted-foreground">Distribute equally among all telecallers</span>
                        </div>
                      </SelectItem>
                      <SelectItem value="workload">
                        <div className="flex flex-col">
                          <span className="font-medium">By Workload</span>
              ...
              <div className="flex flex-wrap gap-2">
                ...
                <DropdownMenu>
                  <DropdownMenuTrigger asChild>
                    <Button size="sm" variant="outline">
                      ...
                      Bulk Assign ({bulkAssignTo.length || '0'})
                    </Button>
                  </DropdownMenuTrigger>
                  <DropdownMenuContent className="w-64">
                    <DropdownMenuLabel>Select Telecallers (Round Robin)</DropdownMenuLabel>
                    <DropdownMenuSeparator />
                    {telecallers.map((telecaller) => (
                        <DropdownMenuCheckboxItem
                            key={telecaller.id}
                            ...
                            onCheckedChange={() => {
                                setBulkAssignTo(prev => 
                                    prev.includes(telecaller.id) 
                                        ? prev.filter(id => id !== telecaller.id)
                                        : [...prev, telecaller.id]
                                )
                            }}
                        >
                            <div className="flex justify-between items-center w-full">
                                {telecaller.full_name}
                                {/* Telecaller Status Badge in Dropdown */}
                                {statusLoading ? (
                                    <span className="h-2 w-2 rounded-full bg-gray-400 animate-pulse" />
                                ) : (
                                    <span 
                                        className={`h-2 w-2 rounded-full ${telecallerStatus[telecaller.id] === 'online' ? 'bg-green-500' : 'bg-red-500'}`} 
                                        title={telecallerStatus[telecaller.id]}
                                    />
                                )}
                            </div>
                        </DropdownMenuCheckboxItem>
                    ...
                    <DropdownMenuSeparator />
                    <DropdownMenuItem 
                        // New handleBulkAssign uses state, no arguments needed
                        onClick={handleBulkAssign}
                        disabled={bulkAssignTo.length === 0}
                        className="cursor-pointer font-medium text-primary-600"
                    >
                        <Save className="mr-2 h-4 w-4" /> Apply Round-Robin Assignment
                    </DropdownMenuItem>
                  </DropdownMenuContent>
                </DropdownMenu>
                {/* *** END: Bulk Assignment Multi-Select DropdownMenu *** */}
                ...
                <DropdownMenu>
                  ...
                  <DropdownMenuContent>
                    ...
                    <DropdownMenuItem onClick={() => handleBulkStatusUpdate('Documents_Sent')}>
                      Documents Sent
                    </DropdownMenuItem>
                    <DropdownMenuItem onClick={() => handleBulkStatusUpdate('Login')}>
                      Login
                    </DropdownMenuItem>
                          ...
                          {lead.assigned_user ? (
                            <div className="flex items-center gap-2">
                              ...
                              {statusLoading ? (
                                  ...
                                  <span 
                                      className={`w-2 h-2 rounded-full ${telecallerStatus[lead.assigned_user.id] === 'online' ? 'bg-green-500' : 'bg-red-500'}`} 
                                      title={telecallerStatus[lead.assigned_user.id]}
                                  />
                              )}
